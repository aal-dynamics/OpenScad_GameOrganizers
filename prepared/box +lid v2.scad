...
module boxWithLid(x = 0.0, y = 0.0, z = 0.0, material = 1.2, eco = [false, false, false, false, true, true], hashSpace = 7, tolerance = 0.4) {
	// eco[0] = front	// eco[1] = left	// eco[2] = back	// eco[3] = right	// eco[4] = bottom	// eco[5] = lid

	translate(v = [x + 10, 0 , 0])
		lid(x = x - tolerance * 2, y = y, z = z, hashSpace = hashSpace);
	...
	difference() {
		...
		translate(v = [material, -material, z - material *2 - tolerance]) 
			cube(size = [x - material *2, material * 3, material * 3]);
	}
	translate(v = [material, 0, z - material * 4 - tolerance])
	...
	translate(v = [x - material * 2, 0, z- material * 4 - tolerance])
